    ...
    const formatted = realisations.map(r => {
      ...
      ? {
          ...
          Montant: montantValide,
           // montant affiché = montant validé seulement
        }
      : null
      };
    });
...
export async function POST(req: Request) {
  try {
    ...
    if (realisation.paiement?.Statut_Paie === StatutPaiement.TOTAL) {
      return NextResponse.json({
        message: "Impossible d'effectuer un paiement : le sous-acte est déjà payé en totalité."
      }, { status: 400 });
    }
